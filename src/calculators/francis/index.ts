 ...
 */
import { calculateFrancis, FrancisResults } from './formulas';
import { collectInput, FrancisOptions } from './io';
 ...
 */
export const printResults = (results: FrancisResults): void => {
  const header = safeColorize('\n=== Francis Turbine Results ===', theme.header);
  console.log(header);
 ...
 */
export const printResultsJson = (results: FrancisResults): void => {
 ...
 */
export const runFrancis = async (options: FrancisOptions = {}): Promise<void> => {
  try {
    const input = await collectInput(options);
    const results = calculateFrancis(
      input.ppg,
      input.npg,
      input.w1,
      input.w2,
