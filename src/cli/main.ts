...
import { runKaplan } from "../calculators/kaplan";
...
// Kaplan Turbine subcommand
program
  .command("kaplan")
  .description("Run Kaplan Turbine calculations")
  .option("--pg <value>", "Pressure gauge reading")
  .option("--w1 <value>", "Weight 1 (kg)")
  .option("--w2 <value>", "Weight 2 (kg)")
  .option("--rpm <value>", "RPM (revolutions per minute)")
  .option("--lhs-mercury <value>", "Left hand side manometer reading")
  .option("--rhs-mercury <value>", "Right hand side manometer reading")
  .option("--json", "Output results in JSON format")
  .action(async (options) => {
    try {
      await runKaplan({
        ...
        w2: options.w2 ? Number(options.w2) : undefined,
        rpm: options.rpm ? Number(options.rpm) : undefined,
        lhs_mercury: options.lhsMercury
          ? Number(options.lhsMercury)
          : undefined,
        rhs_mercury: options.rhsMercury
          ? Number(options.rhsMercury)
          : undefined,
        json: options.json,
      });
    } catch (error) {
...
program
  ...
  .action(async () => {
    ...
    const kaplanDesc = safeColorize("Kaplan Turbine Calculator", theme.info);
    const usage = safeColorize(
      'Use "lab-work <calculator> --help" for usage information.',
      theme.subtle
    );
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      const kaplan = safeColorize("3. Kaplan Turbine", theme.accent);
      ...
      const choice = await prompt.ask(
        safeColorize("\nChoose calculator (1-3): ", theme.prompt)
      );

      switch (choice.trim()) {
        case "1":
          const startingFrancis = safeColorize(
            "Starting Francis Turbine calculator...",
            theme.success
          );
          console.log(`\n${startingFrancis}\n`);
          await runFrancis({});
          break;
        case "2":
          const startingPelton = safeColorize(
            "Starting Pelton Wheel calculator...",
            theme.success
          );
          console.log(`\n${startingPelton}\n`);
          await runPelton({});
          break;
        case "3":
          const startingKaplan = safeColorize(
            "Starting Kaplan Turbine calculator...",
            theme.success
          );
          console.log(`\n${startingKaplan}\n`);
          await runKaplan({});
